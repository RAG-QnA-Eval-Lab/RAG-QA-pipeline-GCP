...
`docs/plan.md`에 Phase 0~6 구현 계획이 정의되어 있다. 구현 시 해당 Phase 섹션을 반드시 참조할 것. `src/` 내 파일 중 `__init__.py` 스텁만 있는 모듈은 아직 미구현 상태이다.

## Commands

```bash
# Setup
pip install -e ".[dev,api,ui,ko,crawl,viz]"
...
- **LiteLLM** 으로 모델 전환: `openai/gpt-4o-mini`, `anthropic/claude-sonnet-4-20250514`, `gemini/gemini-2.0-flash`, `ollama/llama3.2`. 모델 정의는 `config/models.py`의 `MODELS` dict.
...
- **설정 관리**: `config/settings.py`의 `Settings` 클래스 (pydantic-settings). `.env` 파일에서 자동 로드. 임베딩 모델, 청크 크기, top_k 등 기본값이 여기에 정의됨.
- **데이터 소스 설정**: `config/policy_sources.py`의 `POLICY_SOURCES` dict. 각 소스별 URL, 수집 방식, 우선순위, 엔드포인트 정보.
