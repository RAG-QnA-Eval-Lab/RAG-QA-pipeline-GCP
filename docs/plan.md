...
**현재 진행 상황** (2026-04-22):
- Phase 0: 완료 (예산 알림/Gemini API 키 잔여)
...
- Phase 3: 코드 구현 완료. LLM 실호출 미검증 (mock 테스트만)
- QA 데이터셋: 100쌍 생성 완료 (`scripts/generate_qa.py` → `data/eval/qa_pairs.json`)
- GCP 배포: Dockerfile 4종 + GitHub Actions 워크플로 4종 작성 완료. `src/api/main.py` 최소 스텁 (헬스체크만). Secrets 등록/실배포 미완료
...
> **방향 전환**: Vertex AI Model Garden 경유로 전환 시 OpenAI/Anthropic 개별 키 불필요 → GCP SA 인증 하나로 통일.
> 임베딩도 Vertex AI의 `text-embedding-004` 등으로 대체 가능.

| API | 필수 | 용도 | 비용 | Vertex AI 전환 후 |
|-----|------|------|------|------------------|
| OpenAI | 필수 | 임베딩 + GPT-4o-mini/4o | 유료 | Vertex AI 경유 (GCP 크레딧) |
| Google Gemini | 필수 | Gemini 비교 평가 (GCP 크레딧) | GCP 크레딧 | Vertex AI 네이티브 |
| Anthropic | 선택 | Claude 비교 평가 | 유료 | Vertex AI 경유 (GCP 크레딧) |
| Ollama | 선택 | 로컬 LLM (무료) | 무료 | 유지 (로컬 테스트용) |
...
# 전환 예정 (Vertex AI Model Garden — GCP 크레딧 통합 과금)
MODELS = {
    ...
    "claude-sonnet": {"id": "vertex_ai/claude-sonnet-4-20250514", "temperature": 0.0, "max_tokens": 2048},
    "gemini-flash": {"id": "vertex_ai/gemini-2.0-flash", "temperature": 0.0, "max_tokens": 2048},
    "llama3.2": {"id": "vertex_ai/meta/llama-3.1-405b-instruct-maas", "temperature": 0.0, "max_tokens": 2048},
}
```
...
공공데이터포털 (API) →  DataPortalCollector   →  PolicyNormalizer ┤  gs://bucket/policies/raw/
한국장학재단 (HTML) →  KosafCollector        →                  ┤  gs://bucket/policies/processed/
...
**저장소 역할 분리**:
- **GCS (실제 데이터)**: 정책 원본 JSON/PDF, 정규화 문서, FAISS 인덱스
- **MongoDB (메타데이터만)**: 정책 목록, 수집 이력, 상태 관리 (gcs_path로 실제 데이터 참조)

**지속적 데이터 수집 흐름 (이벤트 드리븐)**:
```
Cloud Scheduler (매일 1회)
  → Cloud Run Job #1 (수집기) → GCS (원본 저장) + MongoDB (메타데이터 upsert)
      → GCS 업로드 완료 이벤트
          → Eventarc (GCS Object Finalize 트리거)
              → Cloud Run Job #2 (인덱서) → FAISS 인덱스 재빌드 → GCS 업로드
                  → Cloud Run BE 재시작 시 최신 인덱스 로드
```
Eventarc를 사용하여 수집 완료 후 인덱싱이 자동 체이닝됩니다.
수집 시간이 변동되어도 완료 이벤트 기반으로 동작하므로 안정적입니다.
...
- 일반 분할: 문장 경계 기반 (한국어 `kss`, 영어 기본 분리)
- 기본 512 토큰, 오버랩 50 토큰
...
- [ ] RAGAS 4대 메트릭 자동 산출
- [ ] LLM Judge 동작
- [ ] DeepEval Hallucination Score 산출
- [ ] 3단계 종합 리포트 자동 생성
- [ ] `pytest tests/test_evaluation.py` 통과
...
- [ ] 6개 페이지 모두 동작 (최소 챗봇 + 정책 탐색 + 추천 + 대시보드 4개 필수)
- [ ] 정책 카드에서 상세 정보 확인 가능
- [ ] 챗봇에서 출처 인용 + 신뢰성 점수 표시
- [ ] 필터링/검색으로 원하는 정책 탐색 가능
- [ ] 로컬에서 전체 앱 동작 확인
...
│  └────────┬─────────┘            └──────────────────┘                   │
│           │                                                              │
│  Compute Engine (e2-small)        GCS (실제 데이터 저장소)                 │
│  ┌──────────────────┐            ┌──────────────────────┐               │
│  │  MongoDB   :27017 │            │ policies/raw/         │               │
│  │  Grafana   :3000  │            │ policies/processed/   │               │
│  │  (메타데이터+모니터링)│            │ index/faiss.index     │               │
│  └──────────────────┘            │ index/metadata.pkl    │               │
│        ↑                         └──────────────────────┘               │
│   Compass + Grafana (로컬 Mac)                                           │
│                                                                          │
│  Cloud Scheduler (매일 1회) → Cloud Run Job #1 (수집기)                   │
│        → GCS (원본) + MongoDB (메타데이터)                                 │
│        → Eventarc (GCS 이벤트) → Cloud Run Job #2 (인덱서) → GCS 업로드   │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 설정 | 값 | 이유 |
|------|------|------|
| MongoDB VM | e2-small (~₩25,000/월) | 메타데이터 관리 + Compass GUI |
...
실험 1: 모델 비교 (검색 고정: Hybrid+Rerank)
  GPT-4o-mini / GPT-4o / Claude Sonnet / Gemini Flash / Llama 3.2
  × 100 QA = 500 평가
...
| Gemini Flash | 0.88 | 0.90 | 0.08 | 1.5s |
...
- [ ] Cloud Run에서 6페이지 앱 접근 가능 (또는 HF Spaces 대안)
- [ ] Grafana 대시보드 5개 패널 구성 (Cloud Run, MongoDB, RAG 파이프라인, LLM 비용, 데이터 적재)
- [ ] `/health` 헬스체크 엔드포인트 동작 + Uptime Check 알림 설정
...
| LLM 통합 | LiteLLM + Vertex AI Model Garden | 1줄로 모델 전환, GCP SA 인증 통일, 크레딧 통합 과금 |
| 임베딩 | OpenAI text-embedding-3-small (→ Vertex AI 전환 예정) | $0.02/1M tokens → GCP 크레딧 |
...
| 항목 | 개발 중 | 실험 | 배포 후 (월) | 결제 방식 |
|------|---------|------|-------------|----------|
| 임베딩 | ~$0.50 | - | - | Vertex AI (GCP 크레딧) |
| GPT-4o-mini | ~$1-2 | ~$3-5 | ~$1-3 | Vertex AI (GCP 크레딧) |
| GPT-4o | - | ~$3-5 | - | Vertex AI (GCP 크레딧) |
| Claude | - | ~$3-5 | - | Vertex AI (GCP 크레딧) |
| Gemini Flash | - | ~$0 | - | Vertex AI 네이티브 (GCP 크레딧) |
| Ollama | $0 | $0 | - | 무료 |
| GCP 인프라 (Cloud Run) | ~$0 | ~$0 | ~$0 | **GCP Annual 크레딧** |
| GCP VM (MongoDB) | ~₩25,000/월 | ~₩25,000/월 | ~₩25,000/월 | **GCP Annual 크레딧** |
| GCS (원본+인덱스) | ~$0 | ~$0 | ~$0 | **GCP Annual 크레딧** (수십 MB) |
| **합계** | **~$2-3 + ₩25k** | **~$10-15 + ₩25k** | **~$1-3 + ₩25k** | |

GCP 크레딧 예상 소비: ~₩110,000 (VM ₩50k×2개월 + Cloud Run + Gemini API + GCS).
크레딧 잔여분 충분. **6/19 만료 전에 실험 완료 필수.**
