...
```python
# 현재 (Vertex AI Model Garden + HuggingFace — 전환 완료 2026-04-25)
MODELS = {
...
# GPT/Claude/Gemini → Vertex AI Model Garden (GCP 크레딧 통합 과금)
# Llama 3.3 → HuggingFace Inference API (Vertex AI MaaS 미지원)
```
...
│  │ policies/processed/   │   인덱싱 → FAISS 빌드 → GCS 업로드              │
│  │ index/faiss.index     │   평가 → 결과 저장                              │
│  │ index/metadata.pkl    │                                               │
│  └──────────────────────┘                                               │
└──────────────────────────────────────────────────────────────────────────┘
```
