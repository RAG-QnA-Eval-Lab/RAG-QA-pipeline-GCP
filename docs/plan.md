...
| API | 필수 | 용도 | 비용 |
|-----|------|------|------|
| OpenAI | 필수 | 임베딩 + GPT-4o-mini/4o | 유료 |
| Google Gemini | 필수 | Gemini 비교 평가 (GCP 크레딧) | GCP 크레딧 |
| Anthropic | 선택 | Claude 비교 평가 | 유료 |
| Ollama | 선택 | 로컬 LLM (무료) | 무료 |
| 공공데이터포털 | 필수 | 청년정책 API 데이터 수집 | 무료 |
...
- [ ] `pytest tests/test_ingestion.py tests/test_collectors.py` 통과
- [ ] 수집기로 정책 50건 이상 수집 → GCS 원본 저장 + MongoDB 메타데이터 기록
- [ ] GCS 원본 → 청킹 → 임베딩 → FAISS 인덱스 빌드 동작
- [ ] Compass에서 MongoDB 메타데이터 확인 가능 (정책 목록, gcs_path, 수집 이력)
- [ ] 저장된 청크 수 확인 가능
- [ ] 메타데이터 필터링 검색 동작 (category별)
...
- [ ] Query → Top-5 관련 문서 반환
- [ ] 4가지 검색 전략 모두 동작
- [ ] `pytest tests/test_retrieval.py` 통과
...
- [ ] 정책 질문 → 검색 → 답변 + 출처 인용(정책명, 관할부처) 동작
- [ ] 4개 모델 전환 가능 (GPT-4o-mini, GPT-4o, Gemini Flash, Ollama)
- [ ] `pytest tests/test_generation.py` 통과
...
| LLM 통합 | LiteLLM | 1줄로 모델 전환 (GPT/Claude/Gemini/Llama) |
...
|------|---------|------|-------------|----------|
| 임베딩 | ~$0.50 | - | - | OpenAI |
| GPT-4o-mini | ~$1-2 | ~$3-5 | ~$1-3 | OpenAI |
| GPT-4o | - | ~$3-5 | - | OpenAI |
| Claude | - | ~$3-5 | - | Anthropic |
| Gemini Flash | - | ~$0 | - | **GCP Gemini 크레딧** (₩71,504) |
| Ollama | $0 | $0 | - | 무료 |
