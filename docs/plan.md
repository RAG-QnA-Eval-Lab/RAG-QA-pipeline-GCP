...
**지속적 데이터 수집 흐름**:
```
Cloud Scheduler (매일 1회) → Cloud Run Job → 수집기 → GCS (원본 저장) + MongoDB (메타데이터 upsert)
                                                        ↓ (변경분 감지)
                                                   GCS에서 문서 읽기 → FAISS 인덱스 재빌드 → GCS 업로드
                                                        ↓
                                                   Cloud Run 앱 재시작 시 최신 인덱스 로드
```
...
│                                                                          │
│  Cloud Scheduler (매일 1회) → Cloud Run Job (수집기)                      │
│        → GCS (원본) + MongoDB (메타데이터) → 인덱스 빌드 → GCS 업로드      │
└──────────────────────────────────────────────────────────────────────────┘
```
